...
LABELS = {
    ...
    "cancer": [
        ...
        "Intrauterine Device",
        "Intrauterine Device (years)",
        "Sexually Transmitted Disease",
        "Sexually Transmitted Disease (number)",
        "STDs:condylomatosis",
        ...
        "Sexually Transmitted Disease (Time since last diagnosis)",
        "Dx:Cancer",
        "Dx:CIN",
        "Dx:HPV",
        "Dx",
    ...
    "wheat_seeds": ["area", "perimeter", "compactness", "length", "width", "asymmetry", "groove", "type"],
    "credit": [
        "credit_risk",
        "age",
        "amount",
...
class Dataset:
    ...
    def get_configspace(self):
        ...
        for j in range(self.X.shape[1]):
            ...
            uniform_float_hp = CSH.UniformFloatHyperparameter(
                name, lower=np.min(self.X[:, j]), upper=np.max(self.X[:, j]), log=False
            )
    ...
    def get_input_labels(self, id=None):
        labels = np.array(LABELS[self.dataset_name])
        if id is None:
            return labels[self.input_ids]
        else:
            return str(labels[self.input_ids][id])
